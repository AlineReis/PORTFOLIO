# 🌟 Portfólio - Aline Reis

Bem-vindo ao meu portfólio pessoal! Este projeto foi desenvolvido para apresentar minhas habilidades, projetos e experiência como desenvolvedora em formação.

## 📋 Sobre o Projeto

Este é um site de portfólio responsivo e acessível, criado para destacar minha trajetória profissional e acadêmica na área de tecnologia. O projeto combina design moderno, código limpo e boas práticas de desenvolvimento web.

## 🛠️ Ferramentas Utilizadas

### Design e Prototipagem
- **[Figma](https://www.figma.com/)** - Utilizado para criar o protótipo visual e definir a identidade visual do site, incluindo paleta de cores, layout e espaçamento dos elementos.

### Desenvolvimento
- **HTML5** - Estruturação semântica do conteúdo
- **CSS3** - Estilização pura (sem frameworks), com foco em:
  - Design responsivo
  - Animações e transições suaves
  - Acessibilidade
  - Layout Flexbox

### Versionamento
- **Git** - Controle de versão
- **GitHub** - Hospedagem do código

## 📂 Estrutura do Projeto

```
PROJETO_PORTFOLIO/
│
├── index_final.html          # Arquivo HTML principal
├── style_final.css           # Arquivo CSS com todos os estilos
├── README.md              # Documentação do projeto
│
└── assets/                # Pasta com imagens
    ├── eu.jpg             # Foto de perfil
    ├── python.png         # Ícone Python
    ├── js.png             # Ícone JavaScript
    ├── html.png           # Ícone HTML
    ├── css.png            # Ícone CSS
    ├── git.png            # Ícone Git
    ├── github.png         # Ícone GitHub
    ├── linkedin.png       # Ícone LinkedIn
    └── email.png          # Ícone Email
```

## 🎨 Paleta de Cores

- **Fundo principal**: `#e8e9f3`
- **Seção Sobre**: `#f0f1f8`
- **Seção Projetos**: `#d4d7f0`
- **Cards de Projetos**: `#f4f26e`
- **Rodapé**: `#c8cae8`
- **Texto**: `#000000`
- **Botões**: `#000000` com hover interativo

## 🚀 Como Visualizar o Projeto

### Opção 1: Visualizar Online
Acesse o link do GitHub Pages: [Em breve]

### Opção 2: Executar Localmente

1. Clone este repositório:
```bash
git clone https://github.com/AlineReis/PORTFOLIO
```

2. Abra o arquivo `index_final.html` no seu navegador preferido

## 📱 Responsividade

O site é responsivo e se adapta a diferentes tamanhos de tela:

- **Desktop**: Layout em duas colunas com animações suaves
- **Tablet**: Ajustes de espaçamento e tamanhos
- **Mobile**: Layout em coluna única otimizado para telas pequenas

## ♿ Acessibilidade

O projeto foi desenvolvido seguindo as diretrizes de acessibilidade:

- Tags semânticas HTML5
- Atributos ARIA para leitores de tela
- Contraste adequado de cores
- Foco visível em elementos interativos
- Textos alternativos em imagens
- Navegação por teclado

## 👩‍💻 Sobre Mim

Sou enfermeira, mestre em Bioinformática e desenvolvedora em formação, apaixonada por tecnologia e inovação. Atualmente participo do projeto Alpha Ed/Tech, onde desenvolvo habilidades em programação e desenvolvimento web.

## 📫 Contato

- **GitHub**: [@AlineReis](https://github.com/AlineReis)
- **LinkedIn**: [Aline Reis](https://www.linkedin.com/in/aline-r-53a185123/)
- **Email**: alinesoares_76@hotmail.com

## 📝 Licença

© 2025 Aline Reis. Todos os direitos reservados.

---

⭐ Se você gostou deste projeto, considere dar uma estrela no repositório!